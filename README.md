...
Rollout is not live yet: the documented deployment-lease signing credential
MURAKUMO_CLOUD_SITE_TOKEN_SECRET_2 is absent from the configured kagi vault.
Do not bypass the deployment lease. Deploy cloud-murakumo with its supported
lease workflow, verify /api/v1/preview/models lists waiREALMIX_v11, then deploy
this frontend and verify a full request through /api/free/image. Until then,
the production free selector remains Animagine-only.
